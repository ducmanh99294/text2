function them(x) {
  ...
  var td = tr.children;
  var mssp = td[0].innerHTML;  //Mã sản phẩm lấy từ nội dung (innerHTML) của ô đầu tiên trong hàng.
  var hinh = td[1].children[0].src; //Đường dẫn ảnh sản phẩm lấy từ thuộc tính src của phần tử con đầu tiên trong ô thứ hai của hàng.
  var ten = td[2].innerHTML; //Tên sản phẩm lấy từ nội dung (innerHTML) của ô thứ ba trong hàng.
  var donggia = td[3].children[0].innerHTML; // Đơn giá sản phẩm lấy từ nội dung (innerHTML) của phần tử con đầu tiên trong ô thứ tư của hàng.
  var soluong = td[4].children[0].value; //Số lượng sản phẩm lấy từ giá trị nhập liệu (value) của phần tử con đầu tiên trong ô thứ năm của hàng
  var ttien = parseInt(donggia) * parseInt(soluong); // cái này k biết nữa đóng tiền học lại
  ...
  var giohang_td = document.createElement("td"); // Tạo một phần tử <td> mới
  var giohang_td_ten = document.createTextNode(ten); // Tạo một đối tượng văn bản với giá trị từ biến 'tên'
  giohang_td.appendChild(giohang_td_ten); // Thêm đối tượng văn bản vào phần tử <td>
  giohang_tr.appendChild(giohang_td); // Thêm phần tử <td> vào phần tử 'giohang_tr' (hàng trong bảng)

  var giohang_td = document.createElement("td");
  var giohang_td_dongia = document.createTextNode(donggia);
  ...
  giohang_td_nut.addEventListener("click", function() {
    xoa(this);
  });
...
// Kiểm tra xem nút "Thanh toán" đã được gắn hành động hay chưa
if (!btnThanhToan.hasAttribute("data-action")) {
  btnThanhToan.setAttribute("data-action", "handled");

  btnThanhToan.addEventListener("click", function() {
    var giohang = document.getElementById("giohang");
    var soLuongDonHang = giohang.children.length;

    if (soLuongDonHang > 0) {
      giohang.innerHTML = ""; // Xóa toàn bộ đơn hàng

      // Hiển thị thông báo thanh toán thành công
      alert("Thanh toán thành công. " + soLuongDonHang + " đơn hàng.");
    } else {
      // Hiển thị thông báo không có đơn hàng để thanh toán
...
const nuttat3 = document.querySelector(".nuttatdn3");
const nutshop = document.querySelector(".nutNhan");
...
nutshop.addEventListener("click", function () {
  document.querySelector(".nhan-tin").style.display = "block";
});

nuttat3.addEventListener("click", function () {
  document.querySelector(".nhan-tin").style.display = "none";
});
...
async function taoanh() {
  let apiURL = `https://646ca2087b42c06c3b2bace1.mockapi.io/doan/food`;
...
async function taonguoi() {
  let apiURL2 = `https://646ca2087b42c06c3b2bace1.mockapi.io/doan/nguoi`;
